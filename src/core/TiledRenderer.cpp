...
namespace GLSLPT
{
    ...
    void TiledRenderer::Init()
    {
		...
		denoiserInputFramePtr = new Vector3[frameSize.x * frameSize.y];
		frameOutputPtr = new Vector3[frameSize.x * frameSize.y];
		albedoPtr = new Vector3[frameSize.x * frameSize.y];
		normalPtr = new Vector3[frameSize.x * frameSize.y];
    ...
    void TiledRenderer::Render()
    {
		...
		if (!scene->camera->isMoving && !scene->instancesModified && !scene->hdrModified)
		{
			...
			if (DenoiseMsg.bActive) {
				...
				filter.setImage("color", denoiserInputFramePtr, oidn::Format::Float3, frameSize.x, frameSize.y, 0, 0, 0);
				filter.setImage("albedo", albedoPtr, oidn::Format::Float3, frameSize.x, frameSize.y, 0, 0, 0);
				filter.setImage("normal", normalPtr, oidn::Format::Float3, frameSize.x, frameSize.y, 0, 0, 0);
				filter.setImage("output", frameOutputPtr, oidn::Format::Float3, frameSize.x, frameSize.y, 0, 0, 0);
				...
				glBindTexture(GL_TEXTURE_2D, tileOutputTexture[1 - currentBuffer]);
				glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, frameSize.x, frameSize.y, 0, GL_RGB, GL_FLOAT, frameOutputPtr);
